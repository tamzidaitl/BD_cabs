...
export default function SignupPage() {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      // Registration failed (e.g. duplicate email/phone — EMAIL_TAKEN /
      // PHONE_TAKEN). The message is surfaced via submitError in the danger
      // Alert below; stay on the review step so the user can read it.
      return;
    }
    // Registration auto-logs-in, so send the user straight to their own portal
    // (staff → /dashboard, customer/driver/owner → /app) instead of an interstitial.
  ...
  const canSubmit =
    acceptTerms &&
    (roleInfo?.activeImmediately ? true : ackVerification) &&
    !register.isPending;

  // Codes are unique in COUNTRY_CODES, so matching by code is safe.
  const selectedCountry =
    COUNTRY_CODES.find((c) => c.code === account.countryCode) ??
    COUNTRY_CODES[0];
          ...
          <Card.Body className="p-3 p-md-4">
            <div className="text-center mb-4">
              ...
              <p className="text-muted mb-0">
                One signup for riders, drivers, owners &amp; companies.
              </p>
            </div>
                    ...
                    <Col xs={12} sm={6} key={r.role}>
                      ...
                      >
                        <div className="card-body">
                          ...
                          <Badge
                            bg={r.activeImmediately ? "success" : "warning"}
                            text={r.activeImmediately ? undefined : "dark"}
                          >
                            {r.activeImmediately
                              ? "Active immediately"
                      ...
                      <Card className="border-0 bg-light h-100">
                        <Card.Body>
                          ...
                          <p className="text-muted small mb-2">
                            ...
                            and is activated after we verify:
                          </p>
...
function SummaryCard({
  ...
  const genderLabel =
    GENDERS.find((g) => g.value === account.gender)?.label ?? account.gender;
    ...
    <Card className="border-0 bg-light h-100">
      <Card.Body>
        ...
        <dl className="row mb-0 small">
          {rows.map(([k, v]) => (
            <div
              className="col-12 d-flex justify-content-between py-1 border-bottom"
              key={k}
            >
              <dt className="text-muted fw-normal">{k}</dt>
              <dd className="mb-0 text-end fw-medium text-truncate ps-2">
                {v}
              </dd>
            </div>
          ))}
